...
<body class="min-h-screen flex flex-col antialiased">
    ...
    <nav class="sticky top-0 z-50 backdrop-blur-md bg-slate-900/80 border-b border-slate-800 px-6 py-4">
        <div class="max-w-5xl mx-auto flex items-center justify-between">
            <a href="/" class="flex items-center text-slate-400 hover:text-white transition-colors group">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div>
                    ...
                    <div class="flex items-center space-x-4 text-slate-300">
                        ...
                        <span>•</span>
                        <span class="text-indigo-400 font-bold"><?= htmlspecialchars($emisora['cadena']) ?></span>
                    </div>
                </div>
                ...
                <button class="bg-indigo-600 hover:bg-indigo-500 text-white rounded-full p-4 flex items-center justify-center transition-transform hover:scale-105 shadow-lg shadow-indigo-500/30 group">
                    <div class="text-left mr-4 hidden md:block">
                        ...
                        <p class="font-mono text-xl font-bold"><?= htmlspecialchars($emisora['banda_fm'] ?? $emisora['banda_am']) ?></p>
                    </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <section class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div class="glass-panel rounded-2xl p-5 text-center">
                    ...
                    <p class="text-3xl font-bold text-white"><?= htmlspecialchars($emisora['num_locutores']) ?></p>
                </div>
                <div class="glass-panel rounded-2xl p-5 text-center">
                    <p class="text-slate-500 text-sm mb-1">Programas</p>
                    <p class="text-3xl font-bold text-white"><?= htmlspecialchars($emisora['num_programas']) ?></p>
                </div>
                <div class="glass-panel rounded-2xl p-5 text-center col-span-2 sm:col-span-1">
                    <p class="text-slate-500 text-sm mb-1">Ciudades</p>
                    <p class="text-3xl font-bold text-white"><?= htmlspecialchars($emisora['num_ciudades']) ?></p>
                </div>
            </section>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="glass-panel rounded-2xl p-6">
                ...
                <div class="flex justify-between items-center py-2 border-b border-slate-700/50">
                    ...
                    <span class="font-mono text-indigo-400 font-bold"><?= htmlspecialchars($emisora['banda_fm']) ?></span>
                </div>
                <?php endif; ?>

                <?php if (!empty($emisora['banda_am'])): ?>
                <div class="flex justify-between items-center py-2 border-b border-slate-700/50">
                    <span class="text-slate-300">Amplitud Modulada (AM)</span>
                    <span class="font-mono text-indigo-400 font-bold"><?= htmlspecialchars($emisora['banda_am']) ?></span>
                </div>
                <?php endif; ?>
            </div>
